...
function getStatusConfig(status: string = "active") {
  return STATUS_CONFIG[status] || { bg: "bg-gray-500", dot: "bg-gray-500", label: status };
}
      ...
      <div className="p-3.5 md:p-4">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 text-[11px] md:text-xs text-[#64748b] mb-2.5 md:mb-3">
          <span className="flex items-center gap-1">
            <div className="w-4 h-4 rounded bg-[#f8fafc] flex items-center justify-center">
        ...
        <div className="flex flex-wrap gap-1.5 mb-2.5 md:mb-3">
          {service.tags?.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 bg-[#f8fafc] border border-[#e2e8f0]/60 rounded-md text-[10px] md:text-xs text-[#64748b] font-medium"
            >
              {tag}
            </span>
          ))}
