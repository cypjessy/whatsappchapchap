...
function StatCard({
  ...
  return (
    <div
      className={`
        text-center p-3 md:p-4 bg-[#f8fafc] rounded-xl border border-[#e2e8f0]
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
        {images.map((url, idx) => (
          <button
            ...
            className="aspect-square rounded-xl overflow-hidden bg-[#f8fafc] group relative"
          >
            <img
              src={url}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Hero Image Header */}
        <div className="relative h-52 md:h-64 overflow-hidden shrink-0 bg-[#f8fafc] group">
          {/* Background Image or Fallback */}
          {(() => {
            const heroImage = service.imageUrl || (service.portfolioImages && service.portfolioImages[0]);
            return heroImage ? (
              <img
                src={heroImage}
                alt={service.name}
              ...
              <div className={`w-full h-full bg-gradient-to-br ${service.bgGradient || "from-[#8b5cf6] to-[#7c3aed]"}`}>
                <div className="absolute inset-0 opacity-10 flex items-center justify-center">
                  <span className="text-8xl md:text-9xl select-none">{service.emoji || "✨"}</span>
                </div>
              </div>
            );
          })()}
          ...
          {(service.imageUrl || (service.portfolioImages && service.portfolioImages.length > 0)) && (
            <div className="absolute top-14 right-4 px-2.5 py-1 bg-black/50 backdrop-blur-sm text-white rounded-full text-[10px] font-bold z-10">
              <i className="fas fa-images mr-1.5" />
              {service.portfolioImages?.length || 1} photo{(service.portfolioImages?.length || 1) !== 1 ? "s" : ""}
            </div>
          )}
        </div>
            ...
            <div className="p-4 md:p-6 border-b border-[#e2e8f0]">
              ...
              <div className="flex flex-wrap gap-2">
                {service.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 bg-[#f8fafc] border border-[#e2e8f0] rounded-lg text-xs font-semibold text-[#64748b] flex items-center gap-1.5 hover:border-[#8b5cf6] hover:text-[#8b5cf6] transition-colors"
                  >
                    ...
                    <div
                      ...
                      className={`
                        ...
                        ${isAvailable
                          ...
                          : "bg-[#f8fafc] text-[#94a3b8] opacity-50"
                        }
                      `}
                    >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {service.availability.timeSlots.slice(0, 8).map((time, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 bg-[#f8fafc] border border-[#e2e8f0] rounded-lg text-[11px] font-semibold text-[#64748b]"
                      >
                        {time}
                      </span>
                    ))}
