...
export default function OrdersPage() {
    ...
    async (requestId: string, orderId: string, action: "approve" | "reject") => {
      ...
      try {
        ...
        if (orderDoc) {
          ...
          // Strategy 1: Extract phone from order via getWhatsAppPhone (JID → normalized phone)
          let customerPhone = getWhatsAppPhone({
            customerPhone: orderData.customerPhone || "",
            whatsappJid: orderData.whatsappJid || "",
          });

          // Strategy 2: Fallback to the cancellation request's stored WhatsApp phone
          // (from webhook - guaranteed to be the actual WhatsApp number the customer uses)
          if (!customerPhone || !isValidWhatsAppPhone(customerPhone)) {
            try {
              const cancelDoc = await getDoc(doc(db, "cancellation_requests", requestId));
              if (cancelDoc.exists()) {
                const cancelData = cancelDoc.data();
                customerPhone = cancelData.customerPhone || "";
              }
            } catch (e) {
              console.error("Error reading cancellation request for phone:", e);
            }
          }

          // Strategy 3: Raw normalizePhone as last resort
          if (!customerPhone || !isValidWhatsAppPhone(customerPhone)) {
            customerPhone = normalizePhone(orderData.customerPhone || "");
          }
          ...
          if (customerPhone && isValidWhatsAppPhone(customerPhone)) {
            ...
            console.error("Could not find valid WhatsApp phone for cancellation notification, order:", orderId);
          }
        }
  ...
  return (
    <div className="animate-fadeIn max-w-[1400px] mx-auto">
      ...
      {/* M3 Tabs - All visible on mobile, wraps to multiple rows */}
      <div className="flex flex-wrap gap-2 mb-6 animate-fadeIn px-3" style={{ animationDelay: '0.15s' }}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => handleTabClick(tab.id)}
            className={`h-10 px-3.5 md:px-4 rounded-full text-xs md:text-sm font-medium flex items-center gap-1.5 transition-all duration-200 active:scale-[0.97] ${
          ...
          >
            {tab.icon && <i className={`fas ${tab.icon} text-[10px] md:text-xs opacity-70`} />}
            <span className="truncate max-w-[80px] md:max-w-none">{tab.label}</span>
            <span
              className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10px] font-bold leading-none ${
                (tab.id === "cancellation_requests" && viewMode === "cancellations") ||
                (tab.id !== "cancellation_requests" && activeStatus === tab.id)
                  ? "bg-[#25D366]/15 text-[#1C1B1F]"
