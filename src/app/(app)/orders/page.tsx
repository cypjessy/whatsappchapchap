...
export default function OrdersPage() {
  ...
  const handleCancellationAction = async (requestId: string, orderId: string, action: 'approve' | 'reject') => {
    ...
    // Optional: Add refund note or rejection reason
    const refundNote = window.prompt(isApproving 
      ...
      if (!orderSnap.empty) {
        ...
        if (customerPhone && isValidWhatsAppPhone(customerPhone)) {
          ...
          if (isApproving) {
            ...
            message = `️ *CANCELLATION REQUEST UPDATE* ℹ️\n\n` +
              `Dear ${customerName},\n\n` +
              ...
              `📝 *Note from merchant:* ${refundNote || 'Please contact support if you have questions.'}\n\n` +
              `If you have any concerns, please reply *SUPPORT* and we'll be happy to assist you.\n\n` +
          ...
          console.warn(`[Cancellation] No valid phone number for order ${orderId}`);
