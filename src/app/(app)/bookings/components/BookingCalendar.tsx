...
function getStatusClass(status: string) {
  switch (status) {
    case "confirmed": return "bg-[rgba(37,211,102,0.12)] text-[#10b981] border-[#10b981]/20";
    case "pending": return "bg-[rgba(245,158,11,0.12)] text-[#f59e0b] border-[#f59e0b]/20";
    case "completed": return "bg-[rgba(59,130,246,0.12)] text-[#3b82f6] border-[#3b82f6]/20";
    case "cancelled": return "bg-[rgba(239,68,68,0.12)] text-[#ef4444] border-[#ef4444]/20";
    ...
    <div className="bg-white rounded-xl md:rounded-2xl border border-[#e2e8f0] p-4 md:p-6 mb-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <ShimmerBox className="w-9 h-9 bg-[#e2e8f0] rounded-full" />
          <ShimmerBox className="w-9 h-9 bg-[#e2e8f0] rounded-full" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {Array.from({ length: 35 }).map((_, i) => (
          <ShimmerBox key={i} className="aspect-square bg-[#f1f5f9] rounded-lg" />
        ))}
      </div>
    </div>
  );
    ...
    <div className="flex justify-between items-center mb-4 md:mb-6">
      <div className="flex items-center gap-2 md:gap-4">
        <button
          ...
          className="w-9 h-9 md:w-10 md:h-10 rounded-full border-2 border-[#e2e8f0] flex items-center justify-center text-[#64748b] hover:border-[#8b5cf6] hover:text-[#8b5cf6] hover:bg-[#f5f3ff] active:scale-95 transition-all duration-200"
          aria-label="Previous month"
        >
          <i className="fas fa-chevron-left text-xs md:text-sm" />
        </button>
        <span className="text-base md:text-lg font-extrabold min-w-[140px] md:min-w-[180px] text-center select-none">
...
function CalendarDay({
  ...
  return (
    <button
      ...
      className={`
        relative aspect-square flex flex-col items-center justify-center rounded-lg md:rounded-xl
        text-sm md:text-base font-semibold transition-all duration-200 select-none
        active:scale-90 md:active:scale-95
        ${item.isToday
          ? "bg-gradient-to-br from-[#8b5cf6] to-[#7c3aed] text-white shadow-md shadow-[#8b5cf6]/25"
          : isSelected
            ? "bg-[#ede9fe] text-[#8b5cf6] border-2 border-[#8b5cf6] shadow-sm"
    ...
    <>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2 md:mb-3">
        {DAYS_OF_WEEK.map((day, i) => (
          <div
            key={day}
            className="text-center text-[10px] md:text-xs font-bold text-[#64748b] uppercase tracking-wider py-1.5 md:py-2"
          >
...
function BookingCard({ booking, index, onViewBooking }: {
  ...
  return (
    <div
      className={`
        group bg-[#f8fafc] rounded-xl p-3.5 md:p-4 border border-[#e2e8f0] 
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-2.5 md:gap-3">
          <div className={`
            w-10 h-10 md:w-11 md:h-11 rounded-full bg-gradient-to-br flex items-center justify-center 
          ...
          <div className="min-w-0">
            <div className="font-bold text-sm md:text-base truncate">{booking.client}</div>
        ...
        <span className={`
          px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wide border
  ...
  if (dayBookings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 md:py-16 text-[#64748b] animate-fadeIn">
        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-[#f1f5f9] flex items-center justify-center mb-4">
...
export default function BookingCalendar({
    ...
    return (
      <div className="mb-6">
        ...
        <div className="bg-white rounded-xl md:rounded-2xl border border-[#e2e8f0] overflow-hidden">
          <div className="p-4 md:p-5 border-b border-[#e2e8f0] bg-gradient-to-br from-[#ede9fe] to-[rgba(139,92,246,0.05)]">
            <ShimmerBox className="h-6 bg-[#e2e8f0] rounded-lg w-40" />
          </div>
          <div className="p-4 md:p-5 space-y-3">
            {[0, 1, 2].map((i) => (
              <ShimmerBox key={i} className="h-24 bg-[#f1f5f9] rounded-xl" />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div ref={containerRef}>
      {/* Calendar Grid */}
      <div
        className="bg-white rounded-xl md:rounded-2xl border border-[#e2e8f0] p-3 md:p-6 mb-4 md:mb-6 overflow-hidden"
      ...
      <div className="bg-white rounded-xl md:rounded-2xl border border-[#e2e8f0] overflow-hidden">
        <div className="p-3.5 md:p-5 border-b border-[#e2e8f0] flex justify-between items-center bg-gradient-to-br from-[#ede9fe] to-[rgba(139,92,246,0.05)]">
          <div className="font-bold flex items-center gap-2 text-sm md:text-base">
            <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-[#8b5cf6]/10 flex items-center justify-center">
