...
export default function AdminProtection({ children }: AdminProtectionProps) {
  const { user, loading, isAdmin, isLoadingAdmin } = useAuth();
  ...
  useEffect(() => {
    if (!loading && !isLoadingAdmin && user && !isAdmin) {
      // User is logged in but not admin - redirect to dashboard
      router.push("/dashboard");
    }
  }, [user, isAdmin, isLoadingAdmin, loading, router]);

  // Show branded loading after a brief delay
  useEffect(() => {
  ...
  if ((loading || isLoadingAdmin) && showBrandedLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#25D366] to-[#128C7E]">
        <div className="flex flex-col items-center gap-4">
          ...
          <h2 className="text-white text-xl font-extrabold tracking-tight">Loading Admin Panel...</h2>
