    ...
    <div className="fixed top-4 right-4 z-[3000] flex flex-col gap-2 pointer-events-none">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`
            pointer-events-auto px-4 py-3 rounded-xl shadow-md3-level3 flex items-center gap-3 min-w-[300px]
  ...
  const renderOverview = () => (
    <div className="space-y-5 animate-fadeIn">
              ...
              {allImages.map((img, idx) => (
                <button
                  ...
                  className={`
                    ...
                    ${selectedImage === idx
                      ? "border-[#8b5cf6] shadow-md3-level2 scale-105"
      ...
      <div className="bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9] rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
        ...
        {product.description && (
          <p className="text-sm text-on-surface-variant text-center leading-relaxed max-w-md mx-auto mb-4">
            {product.description}
          </p>
        )}

        <div className="text-center">
          <span className="inline-flex items-baseline gap-1 px-5 py-2 bg-white rounded-xl border border-outline-variant shadow-md3-level1">
            <span className="text-xs text-outline font-bold">KES</span>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {product.variants.map((variant, idx) => (
              <div
                key={idx}
                className="bg-surface-container-lowest rounded-xl p-4 border border-outline-variant hover:border-[#8b5cf6]/30 hover:shadow-md3-level1 transition-all group"
              >
            ...
            {Object.entries(product.filters).map(([key, values]) => {
              if (Array.isArray(values) && values.length > 0) {
                return values.map((value, idx) => (
                  <span
                    key={`${key}-${idx}`}
                    className="px-3 py-1.5 bg-surface-container-lowest border border-outline-variant rounded-full text-xs font-semibold text-on-surface-variant hover:border-[#8b5cf6]/30 transition-all"
                  >
                    {value}
                  </span>
                ));
  ...
  const renderDetails = () => (
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
        ...
        <div className="space-y-2">
          {[
            ...
            <div
              key={idx}
              className="flex items-center justify-between p-3 bg-surface-container-lowest rounded-xl border border-outline-variant/60 hover:border-outline-variant transition-all"
            >
      ...
      <div className="bg-white rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
        ...
        <div className="p-4 bg-surface-container-lowest rounded-xl border border-outline-variant/60 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#ede9fe] flex items-center justify-center">
    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className={`w-10 h-10 rounded-xl ${stockConfig.dot} flex items-center justify-center shadow-md3-level2`}>
        ...
        {/* Progress bar */}
        <div className="mt-2">
          <div className="flex justify-between text-[10px] font-bold text-outline uppercase mb-1.5">
      ...
      {product.variants && product.variants.length > 0 && (
        <div className="bg-white rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-on-surface">Variants</h3>
              <p className="text-[10px] text-outline font-bold">{product.variants.length} total</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {product.variants.map((variant, idx) => (
              <div
                key={idx}
                className="bg-surface-container-lowest rounded-xl p-4 border border-outline-variant hover:border-[#8b5cf6]/30 hover:shadow-md3-level1 transition-all"
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-[#ede9fe] flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl p-5 border border-outline-variant shadow-md3-level1">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-outline font-bold">{specs.length} items</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {specs.map((spec, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`relative overflow-hidden bg-gradient-to-r ${color.from} ${color.to} rounded-xl p-4 border border-outline-variant/60 hover:shadow-md3-level1 transition-all`}
                  >
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-outline-variant shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl overflow-hidden bg-surface-container-lowest flex-shrink-0 border border-outline-variant">
                {!mainImageError && currentImage ? (
                  <img
                    src={currentImage}
                    alt={product.name}
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-on-surface truncate">{product.name}</h3>
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button
                ...
                className="w-9 h-9 rounded-lg hover:bg-surface-container-lowest flex items-center justify-center transition-colors active:scale-90"
                title="Edit (E)"
              >
                <i className="fas fa-edit text-on-surface-variant text-sm" />
              </button>
              <button
                onClick={onClose}
                className="w-9 h-9 rounded-lg hover:bg-surface-container-lowest flex items-center justify-center transition-colors active:scale-90"
